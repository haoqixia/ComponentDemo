...
public class Router {
     ...
     * @param classname 组件名
     */
    public static void registerComponent(@Nullable String classname, @Nullable Application application) {
        ...
        try {
            ...
            applicationLike.onCreate(application);
     ...
     * @param classname 组件名
     */
    public static void unregisterComponent(@Nullable String classname, @Nullable Application application) {
        if (TextUtils.isEmpty(classname)) {
            return;
        }
        if (components.keySet().contains(classname)) {
            components.get(classname).onStop(application);
        ...
        try {
            Class clazz = Class.forName(classname);
            IApplicationLike applicationLike = (IApplicationLike) clazz.newInstance();
            applicationLike.onStop(application);
            components.remove(classname);
        } catch (Exception e) {
            e.printStackTrace();
        }
